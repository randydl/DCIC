...
class WeightCELoss(nn.Module):
    ...
    def forward(self, preds, targets, T=None):
        if T is None:
            loss = F.cross_entropy(preds, targets)
        else:
            clean_probs = F.softmax(preds, dim=1)  # (n, c)
            noise_probs = torch.matmul(clean_probs, T)  # (n, c) * (c, c) -> (n, c)
            clean_probs = torch.gather(clean_probs, dim=1, index=targets.unsqueeze(-1)).squeeze()
            noise_probs = torch.gather(noise_probs, dim=1, index=targets.unsqueeze(-1)).squeeze()
            beta = clean_probs / noise_probs
            beta = beta.clone().detach().requires_grad_(True)
            loss = F.cross_entropy(preds, targets, reduction='none')
            loss = torch.mean(beta * loss)
        return loss
